var HarpaVisualiserBase = require("../common/HarpaVisualiserBase.js");
	...
	*/
	...
	cp.render = function(ctx, face, normalizedVol){
		...
		normalizedVol = normalizedVol || 1.0;
		...
		ctx.scale(3 * normalizedVol.toFixed(6), 1);

		// ctx.scale(3 *  debug_nums[Math.floor((Math.random() * debug_nums.length))], 1);
		// ctx.scale(3 * scaleX, 1);
		...
		// ctx.globalAlpha = alpha;
		ctx.strokeStyle = 'rgb('+this._midrgb[0]+','+this._midrgb[1]+','+this._midrgb[2]+')';
	...
	p.render = function() {
		...
		var normalizedVol = 1 - (this.currentVolume);
