...
float getWaveHeight(vec2 uv, vec2 waveCenter, float waveFront, float waveLength) {
    ...
    if( abs(distToWave - waveFront) < waveLength) {   return (1.0 - sin(abs(distToWave - waveFront)/waveLength * PI * .5)) * .35; }
    else return 0.0;
}
...
void main(void) {
    ...
    float offset = 3.0;
    uv.x += snoise(uv.x*offset, uv.y*offset, time) * noiseOffset;
    uv.y += snoise(uv.y*offset, uv.x*offset, time) * noiseOffset;
